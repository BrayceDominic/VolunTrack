...
const App = () => {
  ...
  useEffect(() => {
    fetch(`http://192.168.100.47:5050/api/supervisors/${supervisorId}/volunteers`)
      .then(res => res.json())
      .then(setVolunteers)
      .catch(console.error);

    fetch(`http://192.168.100.47:5050/api/tasks/${supervisorId}`)
      .then(res => res.json())
      .then(setTasks)
      .catch(console.error);
  }, [supervisorId]);
  ...
  const handleSubmit = () => {
    ...
    fetch('http://192.168.100.47:5050/api/grades/add', {
      method: 'POST',
